...
class SimpleBlock3d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(self.fc0(x), "b h w t c -> b c h w t")
...
class UFNO_Model(Base_Model):
    def __init__(self, in_steps, in_channels, out_channels,
                 modes1, modes2, modes3, width, padding=False, lr=5e-4):
        super().__init__(
            in_steps=in_steps,
        ...
        self.conv1 = SimpleBlock3d(in_channels, out_channels, modes1, modes2, modes3, width)
    ...
    def forward(self, x):
        ...
        if self.hparams.padding:
            ...
            x = self.conv1(x)
            x = x.view(batchsize, size_x+8, size_y+8, size_z+8, self.hparams.out_channels)[..., :-8,:-8,:-8, :]
        else:
            x = self.conv1(x)
